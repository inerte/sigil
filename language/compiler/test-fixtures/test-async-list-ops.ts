...
async function is_even(x) {
  return ((x % 2) === 0);
}

async function sum(acc, x) {
  return (acc + x);
}

async function process_list(xs) {
  return (await (await Promise.all((await await Promise.all((await xs).map(double))).map(async (x) => ({ x, keep: await is_even(x) })))).filter(({ keep }) => keep).map(({ x }) => x)).reduce(async (accPromise, x) => await sum(await accPromise, x), await 0);
}

export async function main() {
  return await process_list([].concat([1], [2], [3], [4], [5]));
}
