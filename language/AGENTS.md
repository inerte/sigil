...
- primitive effects are `Clock`, `Fs`, `FsWatch`, `Http`, `Log`, `Process`, `Pty`, `Random`, `Sql`, `Stream`, `Task`, `Tcp`, `Terminal`, `Timer`, and `WebSocket`
...
- explicit effect annotations are exact after alias expansion: bodies may not omit required effects or declare unused ones just to widen a public contract
